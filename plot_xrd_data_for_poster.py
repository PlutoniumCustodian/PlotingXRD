...
lnthikness= 0.5
ColorPalet = ['#41ad4c', '#4e7b4d', '#4b4b4b']
ylimits = [ 5, 65]
xlimits = [-300, 3e4]
...
fig, ax = plt.subplots(figsize=(7.08,8.5)) #size is in inches
ax.plot(Alco[0,:], Alco[1,:] + 2*off_set, label='Al-Corner', 
...
ax.plot(Cent[0,:], Cent[1,:] + 0*off_set, label='Centroid', 
        linewidth=lnthikness, color=ColorPalet[2])#Graph 3
...
# fig.savefig('Plots/AL_1mL_to_1g_strech.svg', transparent=False, bbox_inches="tight")
...
# with open('Input/Color_Palet_1.txt', 'r') as color_txt:
#     ColorPalet = color_txt.readlines()
...
ax.plot(Pco[0,:], Pco[1,:] + 2*off_set, label='P-Corner', 
        linewidth=lnthikness, color=ColorPalet[0])#Graph 1
